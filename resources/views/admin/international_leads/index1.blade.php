...
@section('content')
    ...
    <div class="panel panel-default">
        ...
        <div class="col-lg-12" style="margin: 10px 10px 10px 0px">
            {{--<br><br><br><br><br>--}}
            <form id="search_form" action="{{ route('international.searchLead') }}" method="POST" role="search">
                {{ csrf_field() }}
...
//console.log(d.que );
            ...
            $(document).on('click', '.deleteLead' , function(event)
            {
                $this = $(this);
                var id = $this.data('id');
                var dataString = {deleteID: id , _token: "{{ csrf_token() }}" , _method: "DELETE" };
                var res = confirm("Are you sure you want to delete this lead?");
                if(res)
                {
                    $.ajax({
                        url: '{{ url('/admin/international') }}' + '/' + id,
                        type: 'DELETE',
                        data: dataString,
                        success: function( response ) {
